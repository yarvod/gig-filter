...
class SpectrumBlock:
    def __init__(self, port_number: int = config.PROLOGIX_ADDRESS, address: int = config.SPECTRUM_ADDRESS):
    ...
    def peak_search(self):
        return self.instr.query(f"CALC:MARK:MAX?")

    def get_peak_freq(self):
        return self.instr.query(f"CALC:MARK:COUN:FREQ?")

    def get_peak_power(self):
        return self.instr.query(f"CALC:MARK:Y?")
